        ...
        <div class="max-w-4xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg mb-6">
                <div class="p-6 text-gray-900">
                    ...
                    @elseif (! $fund->class_code && ! in_array($fund->template, \App\Models\Fund::OVERVIEW_TEMPLATES, true))
                    ...
                    @elseif (empty($availableMonths))
                        <p class="text-sm text-gray-600">No downloaded data for fund code <strong>{{ $fund->fund_code }}</strong>@if ($fund->class_code) class <strong>{{ $fund->class_code }}</strong>@endif yet. The feed is synced daily at 05:00 (or run <code class="text-xs bg-gray-100 px-1 rounded">php artisan funds:sync-data</code>).</p>
                    @else
                        <p class="text-sm text-gray-600 mb-4">Months downloaded from the data feed for fund code <strong>{{ $fund->fund_code }}</strong>@if ($fund->class_code), class <strong>{{ $fund->class_code }}</strong>. Only this class's exports are imported.@endif Importing snapshots a revision first, so you can restore if needed.</p>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <select id="template" name="template" class="block mt-1 w-full border-gray-300 focus:border-indigo-500 focus:ring-indigo-500 rounded-md shadow-sm">
                                        @foreach (['show' => 'Default', 'show-equity' => 'Equity', 'show-flexible' => 'Flexible', 'show-conservative' => 'Conservative', 'show-bond' => 'Bond', 'show-flex-income' => 'Flex Income', 'show-income' => 'Income', 'show-inflation-income' => 'Inflation Linked Income', 'show-domestic' => 'Domestic Balanced', 'show-absolute' => 'Absolute Return', 'show-shariah' => 'Shariah Balanced', 'show-shariah-income' => 'Shariah Income', 'show-international' => 'International', 'show-international-trust' => 'International Trust', 'show-global-equity' => 'Global Equity (Lux)', 'show-feeder' => 'Feeder', 'show-prescient-feeder' => 'Prescient Feeder', 'show-prescient-global-equity' => 'Prescient Global Equity Feeder', 'show-global-equity-feeder' => 'Global Equity Feeder', 'show-hassen-shariah' => 'Hassen Shariah Global Equity', 'show-australian-feeder' => 'Global Equity Australian Feeder', 'show-asia-ex-japan' => 'Asia ex-Japan', 'show-local-overview' => 'Fund Overview: South Africa', 'show-global-overview' => 'Fund Overview: Global'] as $val => $label)
