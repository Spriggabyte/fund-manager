...
<head>
    ...
    <style>
        ...
        /* Income page 1 — portfolio structure + statistics, maturity spread +
           credit exposure, cash chart, performance table, monthly grid
           (mirrors pdf-income.blade.php) */
        .structure-stats-row {
            display: flex;
            gap: 4mm;
            margin: 0 0 3mm 0;
        ...
        .structure-table table td:first-child { width: 54%; }
        ...
        .maturity-spread-block { width: 49%; min-width: 0; padding-right: 4mm; }
        ...
        .portfolio-chart-row { margin: 0 0 2.5mm 0; }
        .portfolio-chart-block { width: 63%; min-width: 0; }
        .overleaf-note {
            font-family: 'Avenir Next', 'Lato', sans-serif;
            font-size: 5.5pt;
            color: var(--dark-grey);
        ...
        /* Page 1 monthly performance grid (825 reference) */
        .monthly-section { margin-top: 2mm; margin-bottom: 1mm; }
        ...
        .page2-footnotes { margin-top: 40mm; }
...
<body x-data="fundEditor()">
    ...
    <div class="page">
        ...
        <div class="content-wrapper">
            ...
            <div class="main-content">
                <!-- Portfolio Structure + Portfolio Statistics (825 reference) -->
                @php
                        ...
                        <table>
                            ...
                            <tbody>
                                {{-- Income display rules (per the published 825 reference,
                                     mirrored in pdf-income.blade.php): the import writes
                                     raw row names (Fund, Benchmark, Fund highest/lowest).
                                     Display order puts a blank spacer before the
                                     highest/lowest rows. Footnotes: Fund⁶,
                                     highest/lowest⁶,⁷; Benchmark has no marker. --}}
                                @php
                                ...
                                @foreach ($perfMainRows as $idx => $row)
                                    <tr class="{{ $idx === 0 ? 'highlight-row' : '' }}">
                                        <td>{!! $decorateIncomeName((string)$row['name']) !!}</td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // signed-off Balanced portfolio chart (show.blade.php / pdf-income.blade.php).
            // The 825 reference ticks every NINE months from the first data point
            // (Oct 22, Jul 23, Apr 24, Jan 25, Oct 25, Jul 26).
            ...
            const renderCashChart = (containerId, data, seriesDefs) => {
                ...
                // fact sheet and the PDF template (pdf-income.blade.php). Keep in sync.
                const maxVal = Math.max(
                    ...data.map(d => Math.max(...seriesDefs.map(s => d[s.key] || 0)))
                );
                // Tight max rounded up to 5 — matches the published 825 chart
                // (see pdf-income.blade.php). Keep in sync.
                const yMax = Math.ceil(maxVal * 1.05 / 5) * 5;

                const dates = data.map(d => d.date);
                const nineMonthTicks = (function () {
