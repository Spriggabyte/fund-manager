...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* =====================================================
           INCOME PAGE 1 — portfolio structure + statistics,
           maturity spread + credit exposure, cash chart,
           performance table, monthly performance grid
           (825 reference: Foord Income Fund Class B2 at 2026-07-31)
           ===================================================== */

        /* Portfolio structure (left) + portfolio statistics (right).
           The income structure table is a single value column headed by the
           month-end date — no SA/FOREIGN split, no foreign-currency rows. */
        .structure-stats-row {
            display: flex;
            gap: 6mm;
            /* Lands the MATURITY/CREDIT headings at the reference y=112.4mm. */
            margin: 0 0 4.3mm 0;
        }
        ...
        /* Reference row shading: Yield darkest, Spread to JIBAR mid, the
           duration rows grey-3; the SA duration group row is medium weight. */
        ...
        /* Maturity spread (left) + credit exposure tables (right). */
        .maturity-credit-row {
            display: flex;
            gap: 6mm;
            /* Lands PORTFOLIO VS BENCHMARK at the reference y=159mm. */
            margin: 0 0 6.5mm 0;
        }
        .maturity-spread-block { width: 49%; min-width: 0; padding-right: 6mm; }
        .credit-block { flex: 1; min-width: 0; }
        ...
        /* Cash chart row — the 825 reference draws the chart alone at ~63%
           of the main-content width, left-aligned. */
        .portfolio-chart-row {
            /* Lands the performance-table title at the reference y=215.9mm. */
            margin: 0 0 6mm 0;
        }
        ...
        /* The numbered footnote block sits between the TER paragraph and
           the footer (no monthly grid on the income page 2, so the gap is
           wide — reference footnotes start ~203mm from the page top). */
        .page2-footnotes {
            margin-top: 94mm;
        }
        ...
        /* Income TIC has four data rows: TER + Transaction costs white,
           the two sub-item rows grey (825 reference). */
        ...
        .footer {
            /* Lands "Please visit our website…" at the reference y=251.2mm. */
            margin-top: 11.7mm;
            padding-top: 5.5mm;
            border-top: none;
            position: relative;
...
<body x-data="fundEditor()">
    ...
    <div class="page">
        ...
        <div class="content-wrapper">
            ...
            <div class="main-content">
                <!-- Portfolio Structure + Portfolio Statistics -->
                @php
                    $structure = $fund->data['mainContent']['assetAllocation'] ?? null;
                    $stats = $fund->data['mainContent']['assetAllocation']['portfolioStatistics'] ?? null;
                @endphp
                @if(isset($structure['rows']) || $stats)
                    <div class="structure-stats-row">
                        @if(isset($structure['rows']))
                            <div class="structure-block">
                                <h3 class="section-heading">{{ $structure['title'] ?? 'PORTFOLIO STRUCTURE %' }}</h3>
                                @if(!empty($structure['subtitle']))
                                    <p class="section-subheading"><span x-data="editableField('mainContent.assetAllocation.subtitle', '{{ addslashes($structure['subtitle']) }}')" @click="editMode && startEdit()" :class="editMode ? 'editable' : ''">{{ $structure['subtitle'] }}</span></p>
                                @endif
                                <div class="table-container structure-table">
                                    <table>
                                        <thead>
                                            <tr>
                                                @foreach ($structure['headers'] ?? ['ASSET CLASS', '', 'CHANGE'] as $header)
                                                    <th>{{ $header }}</th>
                                                @endforeach
                                            </tr>
                                        </thead>
                                            ...
                                            @foreach ($stats['rows'] ?? [] as $rowIndex => $row)
                                                ...
                                                @else
                                                    <tr class="{{ ($row['bold'] ?? false) ? 'stats-bold-row' : '' }}">
                                                        ...
                                                        <td><span x-data="editableField('mainContent.assetAllocation.portfolioStatistics.rows.{{ $rowIndex }}.value', '{{ addslashes($row['value'] ?? '') }}')" @click="editMode && startEdit()" :class="editMode ? 'editable' : ''">{{ $row['value'] ?? '' }}</span></td>
                                                    </tr>
                                                @endif
                                            @endforeach
                                        </tbody>
                        ...
                        @if($credit)
                            <div class="credit-block">
                                ...
                                <div class="credit-tables">
                                    @foreach ([['RATING<sup>3</sup>', $creditRatings, 'ratings'], ['SECTOR', $creditSectors, 'sectors']] as [$creditHeading, $creditRows, $creditKey])
                ...
                <!-- Portfolio vs Benchmark cash chart (alone in its row per the 825 reference) -->
                ...
                @if(isset($fund->data['mainContent']['performanceTable']))
                    ...
                    <div class="table-container performance-table">
                        <table>
                            ...
                            <tbody>
                                {{-- Income display rules (per the published 825 reference):
                                     the import writes raw row names (Fund, Benchmark,
                                     Fund highest/lowest). Display order puts a blank spacer
                                     before the highest/lowest rows. Footnotes: Fund⁶,
                                     highest/lowest⁶,⁷; the Benchmark row carries no marker. --}}
                                @php
                                    ...
                                    $decorateIncomeName = function (string $name) {
                                    ...
                                    $renderPerfRow = function ($row, $highlight) use ($perfColKeys, $fmt, $normaliseSupers, $decorateIncomeName) {
                                        $cells = '<td>'.$normaliseSupers($decorateIncomeName((string)$row['name'])).'</td>';
                ...
                <!-- Monthly Performance (the 825 reference prints this on page 1) -->
                ...
                @if(isset($fund->data['mainContent']['performanceTable']))
                    {{-- The numbered footnotes print on page 2; page 1 carries only
                         the pointer line (825 reference: below the monthly grid). --}}
    ...
    <div class="page page-2">
        <div class="content-wrapper">
            ...
            <div class="fees-content">
                ...
                <!-- Numbered footnotes (825 reference prints them here, above the footer) -->
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // signed-off Balanced portfolio chart (pdf.blade.php). The 825 reference
            // ticks every NINE months from the first data point (Oct 22, Jul 23,
            // Apr 24, Jan 25, Oct 25, Jul 26), LINEAR y-axis from the 100 baseline.
            const renderCashChart = (containerId, data, seriesDefs, legendItemDistance = 40) => {
                ...
                // Tight max rounded up to 5 — the 825 reference places the R143
                // end label ~77% up the axis (yMax ≈ 155), unlike the balanced
                // chart's next-100 rounding.
                const yMax = Math.ceil(maxVal * 1.05 / 5) * 5;
