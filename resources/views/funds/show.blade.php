...
<body class="bg-gray-100 text-gray-900" x-data="fundEditor()">
...
<div class="no-print bg-gray-800 text-white my-4 p-4 rounded-lg flex justify-between items-center">
...
<div class="flex-1">
<div class="bg-red-600 text-white px-3 py-1 mb-3 inline-block font-semibold text-sm">
...
<img src="https://i.imgur.com/83u6IqH.png" alt="Foord Logo" class="h-12">
</div>

<div class="flex flex-col md:flex-row">
<div class="w-full md:w-80 bg-gray-800 p-6 text-xs text-gray-300">
<div class="space-y-4">
@if(isset($fund->data['sidebar']))
@foreach ($fund->data['sidebar'] as $key => $value)
<div>
<h3 class="font-bold text-gray-400 mb-1 uppercase tracking-wider">{{ implode(' ', preg_split('/(?=[A-Z])/', $key, -1, PREG_SPLIT_NO_EMPTY)) }}</h3>
@if (is_array($value))
<div class="equity-indicator h-4 rounded my-2"></div>
<p>
...
<th class="border border-gray-300 px-3 py-2 font-bold {{ $loop->first ? 'text-left' : 'text-center' }}">
...
<div class="p-1">
<h2 class="text-xs font-bold mb-2">INVESTMENT STRATEGY VS SA INFLATION</h2>
<canvas id="inflationChart"></canvas>
</div>
<div class="p-1">
<h2 class="text-xs font-bold mb-2">PORTFOLIO PERFORMANCE VS BENCHMARK</h2>
...
<th class="border border-gray-300 px-2 py-2 font-bold {{ $loop->first ? 'text-left' : 'text-center' }}">
...
<tr class="bg-white">
<td class="border border-gray-300 px-2 py-2 {{ $loop->first ? 'font-bold' : '' }}">
<span x-data="editableField('mainContent.performanceTable.rows.{{ $rowIndex }}.name', '{!! addslashes($row['name']) !!}')"
@click="editMode && startEdit()"
:class="editMode ? 'editable' : ''"
x-html="value"></span>
</td>
<td class="border border-gray-300 px-2 py-2 text-center {{ $loop->first ? 'font-bold' : '' }}">
...
<h2 class="text-base font-bold mb-4 text-white">
...
@endforeach
<tr>
<td class="py-1 pt-3 font-semibold">
...
<img src="https://i.imgur.com/83u6IqH.png" alt="Foord Logo" class="h-8 mx-auto mt-4">
</div>
@endif
</div>
</div>
...
datasets: [
{ label: 'Composite', data: inflationData.map(item => item.composite), borderColor: '#DC2626', backgroundColor: 'transparent', borderWidth: 2, tension: 0.1, pointRadius: 0, order: 1 },
{ label: 'Excess', data: inflationData.map(item => item.excess), borderColor: 'transparent', backgroundColor: '#1F2937', fill: '+2', order: 2 },
{ label: 'Inflation', data: inflationData.map(item => item.inflation), borderColor: 'transparent', backgroundColor: '#9CA3AF', fill: '+1', order: 3 },
{ label: '5% Hurdle', data: inflationData.map(item => item.hurdle), borderColor: 'transparent', backgroundColor: '#6B7280', fill: 'origin', order: 4 }
]
},
options: {
...
plugins: {
legend: { position: 'bottom', labels: { usePointStyle: true, boxWidth: 6, padding: 20, font: { size: 10 } } },
...
y: { min: -10, max: 35, ticks: { stepSize: 5, font: { size: 10 }, callback: (v) => v + '%' }, grid: { borderDash: [0], color: '#e0e0e0' } }
}
}
});
...
datasets: [
{ label: 'Fund', data: portfolioData.map(item => item.fund), borderColor: '#DC2626', backgroundColor: 'transparent', borderWidth: 2, tension: 0.1, pointRadius: 0 },
{ label: 'Benchmark', data: portfolioData.map(item => item.benchmark), borderColor: '#1F2937', backgroundColor: 'transparent', borderWidth: 2, tension: 0.1, pointRadius: 0 }
]
},
options: {
responsive: true, maintainAspectRatio: false,
...
plugins: {
legend: { position: 'bottom', labels: { usePointStyle: true, boxWidth: 6, padding: 20, font: { size: 10 } } },
tooltip: { mode: 'index', intersect: false, callbacks: { label: (c) => `${c.dataset.label}: R ${c.parsed.y.toLocaleString()}` } }
},
scales: {
x: { grid: { display: false }, ticks: { font: { size: 10 }, maxRotation: 0, autoSkip: true, maxTicksLimit: 7 } },
y: { type: 'logarithmic', ticks: { font: { size: 10 }, callback: (v, i, vals) => (v === 100 || v === 1000 || v === 10000) ? 'R ' + v.toLocaleString() : null }, grid: { borderDash: [0], color: '#e0e0e0' } }
}
}
});
</script>
