...
<head>
    ...
    <style>
        ...
        /* Bond page 1 — maturity chart, statistics, credit exposure
           (mirrors pdf-bond.blade.php) */
        .maturity-section { margin-bottom: 3mm; }
        ...
        .stats-section { margin-bottom: 3mm; }
        ...
        .stats-table table td:first-child { width: 46%; }
        ...
        .credit-block { width: 47%; min-width: 0; }
        ...
        /* Page 2 monthly performance grid */
        .monthly-section { margin-bottom: 4mm; }
        .monthly-table table th { font-size: 5.5pt; padding: 1mm 0.8mm; }
        .monthly-table table td { font-size: 6pt; padding: 0.8mm 0.8mm; text-align: right; }
        .monthly-table table td:first-child { text-align: left; }
        .monthly-table table th:first-child { width: 7%; }
        .page2-footnotes { margin-top: 8mm; }
        ...
        /* Conservative reference: the "Foord global funds" subhead is an
           unshaded plain row; the two global fund rows below it are pink. */
        .page .fee-rates-table tr.global-funds-header td {
            background-color: transparent !important;
            text-align: left;
        }

        .page .fee-rates-table tr.sub-item td {
            background-color: var(--naartjie-20);
        }
...
<body x-data="fundEditor()">
                    ...
                    <div class="maturity-section">
                        ...
                        @if(!empty($maturity['subtitle']))
                            <p class="section-subheading">
                                <span x-data="editableField('mainContent.charts.maturityData.subtitle', '{{ addslashes($maturity['subtitle']) }}')"
                                      @click="editMode && startEdit()"
                                      :class="editMode ? 'editable' : ''"
                                      x-text="value"></span>
                            </p>
                                    ...
                                    @foreach ($stats['rows'] ?? [] as $rowIndex => $row)
                                        ...
                                        @else
                                            <tr>
                                                ...
                                                @foreach (['fund', 'benchmark', 'relative'] as $colKey)
                                                    <td>
                                                        <span x-data="editableField('mainContent.assetAllocation.portfolioStatistics.rows.{{ $rowIndex }}.{{ $colKey }}', '{{ addslashes($row[$colKey] ?? '') }}')"
                                                              @click="editMode && startEdit()"
                                                              :class="editMode ? 'editable' : ''"
                                                              x-text="value"></span>
                                                    </td>
                        ...
                        <table>
                            ...
                            <tbody>
                                {{-- Bond fund display rules (per the signed-off 826 reference
                                     PDF, mirrored in pdf-bond.blade.php): the import writes raw
                                     row names (Fund, Benchmark, Fund highest/lowest). Display order
                                     puts a blank spacer before the highest/lowest rows. Footnotes:
                                     Fund⁶, highest/lowest⁶,⁷; the Benchmark row has no marker. --}}
                                @php
                                ...
                                @foreach ($perfMainRows as $idx => $row)
                                    <tr class="{{ $idx === 0 ? 'highlight-row' : '' }}">
                                        <td>{!! $decorateBondName((string)$row['name']) !!}</td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // the signed-off 826 reference — y-axis ticked every 10%, no
            // gridlines, two-line category labels (bucket + quarterly change),
            // centred square-marker legend below. Keep identical to the PDF
            // template (pdf-bond.blade.php).
            if (maturityData.length > 0) {
                ...
                Highcharts.chart('maturityChart', {
                    chart: { type: 'column', backgroundColor: 'transparent', spacing: [4, 4, 2, 4], animation: false },
                    title: { text: null },
                    xAxis: {
                        ...
                        tickWidth: 0,
                        labels: {
                            style: { fontSize: '7px', color: colors.darkGrey, textAlign: 'center' },
                            formatter: function () {
                    ...
                    yAxis: {
                        ...
                        gridLineWidth: 0,
                        lineWidth: 1, lineColor: '#535353',
                        tickWidth: 1, tickLength: 3, tickColor: '#535353',
                        minorTickInterval: 5, minorTickWidth: 1, minorTickLength: 3,
                        minorTickColor: '#535353', minorGridLineWidth: 0,
                        plotLines: [{ value: 0, color: '#535353', width: 1, zIndex: 4 }],
                        labels: {
                    ...
                    legend: {
                        ...
                        symbolWidth: 7, symbolHeight: 7, symbolRadius: 0,
                        itemDistance: 18, margin: 6, padding: 0,
                        squareSymbol: true,
                    },
            ...
            // The portfolio chart is the cash-value spline chart in the exact style of the
            // signed-off Balanced portfolio chart (show.blade.php / pdf-bond.blade.php).
            // The 826 reference ticks every SIX months from the first data point
            // (Oct 22, Apr 23, … — the short history quarters the balanced pitch).
            ...
            const renderCashChart = (containerId, data, seriesDefs) => {
                ...
                // fact sheet and the PDF template (pdf-bond.blade.php). Keep in sync.
