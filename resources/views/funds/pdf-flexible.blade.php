...
<head>
    ...
    <style>
        ...
        * {
        ...
        /* Reference indents the sub-item fund names ~1.4mm past the other labels */
        .fee-rates-table tr.sub-item td:first-child {
            padding-left: 3mm;
        }

        /* Flexible reference: the closing "Foreign assets" row is pink */
        .fee-rates-table tbody tr:last-child td {
            background-color: var(--naartjie-20);
        }
...
<body>
                        ...
                        <table>
                            ...
                            <tbody>
                                ...
                                {{-- Flexible fund display rules (per the signed-off 817 reference
                                     ...
                                     Comparator 2, Fund highest/lowest). Display order puts the
                                     comparator (the FTSE/JSE All Share index) after the Benchmark,
                                     then a blank spacer, then the highest/lowest rows. Footnotes:
                                     Fund³, Benchmark⁴, FTSE/JSE All Share⁵, highest/lowest³,⁶. --}}
                                @php
                                    $perfMainRows = [];
                                    $perfHighLowRows = [];
                                    ...
                                    $renderPerfRow = function ($row, $highlight) use ($perfColKeys, $fmt, $normaliseSupers, $decorateFlexibleName) {
                                        $cells = '<td>'.$normaliseSupers($decorateFlexibleName((string)$row['name'])).'</td>';
                    ...
                    @if(isset($fund->data['mainContent']['performanceTable']['footnotes']))
                        <div class="footnotes">
                            @foreach ($fund->data['mainContent']['performanceTable']['footnotes'] as $footnote)
                                {{-- Reference sets the closing note WITHOUT a colon:
                                     "Note Totals may not cast perfectly due to rounding". --}}
                                <p>{!! $normaliseSupers(preg_replace('/^Note:\s*/u', 'Note ', $footnote)) !!}</p>
                            @endforeach
                        </div>
                    @endif
                @endif
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // Investment Strategy vs Reg 28 Portfolios — the longer "Foord
            // Regulation 28" legend label needs a tighter item gap to stay on
            // one line like the reference (ref gap ≈32 CSS px vs 49 on the
            // right-hand chart).
