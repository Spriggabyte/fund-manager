...
<body x-data="fundEditor()">
                        ...
                        <table>
                            ...
                            <tbody>
                                {{-- Conservative fund display rules (per the signed-off 818 reference
                                     PDF, mirrored in pdf-conservative.blade.php): the import writes raw
                                     row names (Fund, Benchmark, Fund highest/lowest). Display order puts
                                     a blank spacer before the highest/lowest rows. Footnotes: Fund³,
                                     Benchmark⁴, highest/lowest³,⁵. --}}
                                @php
                                ...
                                @foreach ($perfMainRows as $idx => $row)
                                    <tr class="{{ $idx === 0 ? 'highlight-row' : '' }}">
                                        <td>{!! $decorateConservativeName((string)$row['name']) !!}</td>
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function () {
            ...
            // The rolling one-year return bar chart (left) per the signed-off 818
            // reference: solid naartjie columns, y-axis ticked every 5% from just
            // below the series minimum, no gridlines, a dark baseline at 0%, and
            // x labels every second December. Keep identical to the PDF template
            // (pdf-conservative.blade.php).
            if (rollingData.length > 0) {
                const values = rollingData.map(d => d.value);
                const yMin = Math.floor(Math.min(...values, 0));
                const yTicks = [];
                for (let t = yMin; t < Math.max(...values) + 5; t += 5) yTicks.push(t);
                ...
                Highcharts.chart('rollingChart', {
                    chart: { type: 'column', backgroundColor: 'transparent', spacing: [4, 4, 4, 4], animation: false },
                    title: { text: null },
                    xAxis: {
            ...
            // signed-off Balanced portfolio chart (show.blade.php / pdf-conservative.blade.php).
            ...
            const renderCashChart = (containerId, data, seriesDefs) => {
                ...
                // fact sheet and the PDF template (pdf-conservative.blade.php). Keep in sync.
