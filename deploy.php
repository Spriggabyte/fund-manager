...
// NOTE: after the symlink switch, PHP-FPM's OPcache can keep executing the
// PREVIOUS release (stale __DIR__ paths → @vite reads the old manifest and
// pages reference hashed assets that 404). The deploy user has no sudo, so
// this deploy cannot reload FPM itself — an admin must run
// `sudo systemctl reload php8.3-fpm` after deploying if stale assets appear.
